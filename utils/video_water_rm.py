...
TEMP_VIDEO=f'./{int(time.time())}_temp.mp4'
if os.path.exists(TEMP_VIDEO):
    os.remove(TEMP_VIDEO)

class WatermarkRemover():

  def __init__(self,VIDEO_PATH, OUTPUT_PATH,threshold: int, kernel_size: int):
  ...
  def remove_video_watermark(self):
    ...
    mask = None
      ...
      #生成水印蒙版
    mask = self.generate_watermark_mask(VIDEO_PATH)
    ...
    #封装视频
    self.merge_audio(VIDEO_PATH, OUTPUT_PATH, TEMP_VIDEO)
...
if __name__ == '__main__':
  #去除视频水印
  VIDEO_PATH=r"D:\projects\py_win\utils\video.mp4"
  OUTPUT_PATH=r"D:\projects\py_win\utils\video_without_water.mp4"
